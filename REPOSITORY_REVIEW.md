...
| 5   | **Completed 2026-07-14.** The packaged extension now includes verified license text for the copied VS Code code and every bundled runtime dependency. Package validation fails if the root notice file is omitted.                                                                                                                       | Distribution / Compliance  | High before publishing | S      | Low      | High       | [`License.txt`](License.txt), [`check-package-contents.mjs`](scripts/check-package-contents.mjs), [`.vscodeignore`](.vscodeignore).                                                                                                                                                         |
...
Recommended landing order: #1–#3, #5, #6, and #8 are complete. Address the remaining refresh hot path in #4 with regression tests.
...
- Vitest — fourteen unit cases passed.
- VS Code 1.125 Extension Host — extension activation and public command registration passed.
- TypeScript and the production build — passed.
- `vsce ls` — passed and listed the expected runtime files and root third-party license notice.
